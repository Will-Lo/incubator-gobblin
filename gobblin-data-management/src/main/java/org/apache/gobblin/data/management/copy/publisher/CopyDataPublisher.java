...
public class CopyDataPublisher extends DataPublisher implements UnpublishedHandling {
  ...
  public CopyDataPublisher(State state) throws IOException {
    ...
    this.writerOutputDir = state.getPropAsBoolean(ConfigurationKeys.USE_DATASET_LOCAL_WORK_DIR) ?
        null : new Path(state.getProp(ConfigurationKeys.WRITER_OUTPUT_DIR));
  ...
  private void publishFileSet(CopyEntity.DatasetAndPartition datasetAndPartition,
    ...
    Path writerOutputDir = state.getPropAsBoolean(ConfigurationKeys.USE_DATASET_LOCAL_WORK_DIR) ?
        new Path(sampledWorkUnitState.getProp(ConfigurationKeys.WRITER_OUTPUT_DIR)) : this.writerOutputDir;
